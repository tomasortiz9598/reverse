TARGET = reverse # Definimos la tarea que va a ejecutar make por defecto Y el nombre del binario final
...
$(TARGET): $(OBJECTS) # Le pasamos a la tarea los objects ya compilados
	$(CC) $(OBJECTS) $(CFLAGS)  -o $@ # Usamos el compilador para linkear los objects a las bibliotecas y tener nuestro binario
